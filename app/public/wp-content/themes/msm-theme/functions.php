    ...
    function msm_card_shortcode($atts) {
        $atts = shortcode_atts(array(
        'title'   => 'Título de ejemplo',
        'desc'    => '',
        'link'    => '#',
        'variant' => '1', // String, no número
        'height'  => '',  // Ej: '200px'
        ), $atts, 'msm_card');
    
        ob_start();
        
        $variant = trim(strval($atts['variant']));// Asegura que sea string
        $height_style = !empty($atts['height']) ? 'min-height:' . esc_attr($atts['height']) . ';' : '';
    
        switch ($variant) {
    
        case '3': // Variante tramite
            ?>
            <div class="col-12">
            <a href="<?php echo esc_url($atts['link']); ?>">
                <div class="card-tramite d-flex border rounded overflow-hidden" style="min-width: 250px; <?php echo $height_style; ?>">
                <div style="width: 24px; background-color: #00B6ED;"></div>
                <div class="p-3">
                    <h5 class="mb-1"><?php echo esc_html($atts['title']); ?></h5>
                    <?php if (!empty($atts['desc'])) : ?>
                    <p class="mb-0 text-secondary"><?php echo esc_html($atts['desc']); ?></p>
                    <?php endif; ?>
                </div>
                </div>
            </a>
        ...
        case '1':
        default:
            $variant_class = 'card-v' . $variant;
            ?>
            <div class="col-12">
            <a href="<?php echo esc_url($atts['link']); ?>" class="text-decoration-none <?php echo esc_attr($variant_class); ?>">
                <div class="card d-flex flex-row overflow-hidden" style="<?php echo $height_style; ?>">
                <div class="card-barra" style="width: 24px; background-color: #00B6ED;"></div>
                <div class="card-body p-4 pe-2 d-flex flex-column justify-content-center">
                    <h6 class="card-title"><?php echo esc_html($atts['title']); ?></h6>
    
                    <?php if ($variant === '2' && !empty($atts['desc'])) : ?>
                    <p class="card-text fz-14 mb-0"><?php echo esc_html($atts['desc']); ?></p>
